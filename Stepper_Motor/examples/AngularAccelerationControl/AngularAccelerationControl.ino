...
void setup() {
  ...
  Motor1.setAngularAcceleration(0.2);       // установка ускорения 0.2 об/сек^2
  Motor1.goAngularPosition(360*2);          // перемещение в позицию 720 градусов
  delay(Delay);                             // задержка

  Motor1.setAngularAcceleration(0.5);       // установка ускорения 0.5 об/сек^2
  Motor1.goAngularPosition(360*4);          // перемещение в позицию 1440 градусов
  delay(Delay);                             // задержка

  Motor1.setAngularAcceleration(1);         // установка ускорения 1 об/сек^2
  Motor1.goAngularPosition(360*6);          // перемещение в позицию 2160 градусов
  delay(Delay);                             // задержка

  Motor1.setAngularAcceleration(2);         // установка ускорения 2 об/сек^2
  Motor1.goAngularPosition(360*8);          // перемещение в позицию 2880 градусов
  delay(Delay);                             // задержка

  Motor1.setAngularAcceleration(4);         // установка ускорения 4 об/сек^2
  ...
  Motor1.setAngularAcceleration(4);         // установка ускорения 4 об/сек^2
  Motor1.goAngularPosition(-2880);          // перемещение на -2880 градусов
  Serial.println(Motor1.getAngularPosition());  // Получаем конечный угол в абсолютной системе
  */
}
